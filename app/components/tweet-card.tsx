...
export type Comment = {
  ...
  avatar_url?: string; avatar_color?: string; edited?: boolean;
};
...
export const OFFICIAL_LOGO     = "/logo.jpeg";
...
export const avatarGrad = (n: string) => { const [a,b] = GRADS[n.charCodeAt(0) % GRADS.length]; return `linear-gradient(135deg,${a},${b})`; };
...
/* ─── useTick: force re-render every N seconds ─── */
export function useTick(ms = 10000) {
...
/* ─── Badge ─── */
export const Badge = ({ size = 16 }: { size?: number }) => (
...
/* ─── UAv ─── */
export function UAv({ username, size = 42, avatarUrl, avatarColor }: { username: string; size?: number; avatarUrl?: string; avatarColor?: string }) {
  ...
  const bg = avatarColor || avatarGrad(username);
  return <div className="av" style={{ width: size, height: size, background: bg, fontSize: size*0.38 }}>{initial(username)}</div>;
}
...
export function ReplyBox({ assumptionId, addComment, targetUsername, profile, parentId, onClose }: {
  ...
  targetUsername: string; profile: Profile | null; parentId: string | null; onClose?: () => void;
}) {
  const [t, setT] = useState("");
  const submit = () => { if (!t.trim()) return; addComment(assumptionId, t, parentId); setT(""); onClose?.(); };
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", borderTop:"1px solid var(--border2)", background:"var(--surface)" }}>
      <Avatar profile={profile} size={26} />
      <input
        style={{ flex:1, background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:999, outline:"none", padding:"6px 14px", fontSize:13, color:"var(--text)", fontFamily:"inherit" }}
        ...
        autoFocus
      />
      {onClose && <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--muted)", fontSize:18, padding:"0 4px" }}>×</button>}
...
/* ─── Add comment box ─── */
export function AddCommentBox({ assumptionId, addComment, targetUsername, profile, onOpen }: {
  assumptionId: string; addComment: (aid: string, t: string, pid: string | null) => void;
  targetUsername: string; profile: Profile | null; onOpen?: () => void;
}) {
  return (
    <button onClick={() => onOpen?.()} style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 16px", background:"none", border:"none", cursor:"pointer", color:"var(--muted)", fontSize:13, fontFamily:"inherit", width:"100%", borderTop:"1px solid var(--border2)" }}>
...
/* ─── Comment node ─── */
export function CommentNode({ comment: c, allComments, isAdmin, profile, assumptionId, onDelete, onAdd, onEdit, depth, activeReply, setActiveReply }: {
  ...
  onEdit: (id: string, text: string) => void; depth: number;
  activeReply?: string|null; setActiveReply?: (v: string|null) => void;
}) {
  useTick();
      ...
      <div className="comment-item">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {editing ? (
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginTop:4 }}>
              <textarea value={editText} onChange={e => setEditText(e.target.value.slice(0,280))} rows={2}
                style={{ background:"var(--bg)", border:"1px solid var(--red)", borderRadius:8, padding:"6px 10px", fontSize:13, color:"var(--text)", fontFamily:"inherit", outline:"none", resize:"none", width:"100%" }} />
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={() => { onEdit(c.id, editText); setEditing(false); }} style={{ background:"var(--red)", border:"none", borderRadius:999, color:"#fff", fontSize:11, fontWeight:600, padding:"4px 12px", cursor:"pointer", fontFamily:"inherit" }}>Salva</button>
                <button onClick={() => { setEditing(false); setEditText(c.text); }} style={{ background:"none", border:"1px solid var(--border)", borderRadius:999, color:"var(--muted)", fontSize:11, fontWeight:600, padding:"4px 12px", cursor:"pointer", fontFamily:"inherit" }}>Annulla</button>
              </div>
            </div>
          ) : (
        ...
        {profile && profile.username === c.username && (
          <div style={{ display:"flex", gap:4, alignSelf:"flex-start" }}>
            <button className="act" style={{ color:"var(--muted)", padding:"2px 6px", minWidth:"unset" }} onClick={() => setEditing(true)} title="Modifica">
            ...
            <button className="act del" style={{ padding:"2px 6px", minWidth:"unset" }} onClick={() => onDelete(c.id)} title="Elimina">
      ...
      {replying && (
        <div style={{ paddingLeft: depth < 3 ? 52 : 20, background:"rgba(245,240,232,0.5)", borderTop:"1px solid var(--border2)" }}>
          <ReplyBox assumptionId={assumptionId}
            ...
            profile={profile} parentId={c.id} onClose={() => setReplying(false)} />
        </div>
      )}
      {children.length > 0 && (
        <div className="comment-children" style={{ marginLeft: depth < 3 ? 36 : 12 }}>
          {children.map(child => (
            <CommentNode key={child.id} comment={child} allComments={allComments}
              ...
              onDelete={onDelete} onAdd={onAdd} onEdit={onEdit} depth={depth + 1}
...
/* ─── Tweet card ─── */
export function TweetCard({ a, comments, isAdmin, profile, onLike, onDelete, onPin, onDeleteComment, onAddComment, onEditPost, onEditComment }: any) {
  useTick();
        ...
        <div className="tweet-col" onClick={e => e.stopPropagation()}>
          ...
          {editing ? (
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:8 }} onClick={e => e.stopPropagation()}>
              <textarea value={editText} onChange={e => setEditText(e.target.value.slice(0,280))} rows={3}
                style={{ background:"var(--bg)", border:"1px solid var(--red)", borderRadius:10, padding:"8px 12px", fontSize:14, color:"var(--text)", fontFamily:"inherit", outline:"none", resize:"none", width:"100%" }} />
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={() => { onEditPost(a.id, editText); setEditing(false); }} style={{ background:"var(--red)", border:"none", borderRadius:999, color:"#fff", fontSize:12, fontWeight:600, padding:"5px 14px", cursor:"pointer", fontFamily:"inherit" }}>Salva</button>
                <button onClick={() => { setEditing(false); setEditText(a.text); }} style={{ background:"none", border:"1px solid var(--border)", borderRadius:999, color:"var(--muted)", fontSize:12, fontWeight:600, padding:"5px 14px", cursor:"pointer", fontFamily:"inherit" }}>Annulla</button>
              </div>
            </div>
          ) : (
          ...
          <div className="abar">
            ...
            {profile && profile.username === a.username && !isAdmin && (
              <>
                <button className="act" style={{ color:"var(--muted)" }} onClick={e => { e.stopPropagation(); setEditing(true); }} title="Modifica">
        ...
        <div className="comments-area">
          {roots.map((c: Comment) => (
            <CommentNode key={c.id} comment={c} allComments={comments}
              ...
              onDelete={onDeleteComment} onAdd={onAddComment} onEdit={onEditComment} depth={0}
              activeReply={activeReply} setActiveReply={setActiveReply} />
          ))}
          {activeReply === null && (
            <AddCommentBox assumptionId={a.id} addComment={(aid,t,pid) => { onAddComment(aid,t,pid); setActiveReply(null); }}
              targetUsername={displayFor(a.username, a.display_name)}
              profile={profile} onOpen={() => setActiveReply("main")} />
          )}
          {activeReply === "main" && (
            <ReplyBox assumptionId={a.id} addComment={(aid,t,pid) => { onAddComment(aid,t,pid); setActiveReply(null); }}
              targetUsername={displayFor(a.username, a.display_name)}
              profile={profile} parentId={null} onClose={() => setActiveReply(null)} />
          )}
        </div>
      )}
    </div>
